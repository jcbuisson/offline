...
import { getStableListRef } from "/src/use/useStable"
import { horseList } from "/src/use/useHorse"

const emit = defineEmits(['select'])

// ref
const stableListRef = getStableListRef()

const nodesX = computed(() => {
   const nodeList = []
   const stableNodes = stableListRef.value || []
...
const linksX = computed(() => {
   if (!stableListRef.value || !stableListRef.value.length) return []
